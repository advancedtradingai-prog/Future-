...
TELEGRAM_TOKEN = "8607936076:AAG46qqHCzP2SaA7VaI6OpTHX0YzT-GdvZk"
...
class AdaptiveConfig:
   ...
   adx_filter_low: float = 18.0
   adx_filter_medium: float = 22.0
   max_consecutive_losses: int = 3
   weight_freeze_trades: int = 50
...
class MetaLearningEngine:
   ...
   def adapt_to_market_regime(self, current_regime: str, trade_history: List[Dict]):
       ...
       if recent_losses >= self.config.max_consecutive_losses:
           ...
           self.config.min_confidence = min(75.0, self.config.min_confidence * 1.05)
       ...
       if win_rate > 0.5:
           ...
           self.config.risk_per_trade = np.clip(kelly_f * 0.5, 0.0075, 0.01)
       ...
       for i in range(2, min(len(df), 100)):
           try:
               ...
               if current["close"] > current["open"] and momentum > body_size * 0.3:
                   ...
                   ob = {"type": "bullish", "level": current["low"], "strength": strength, "valid": True, "fresh": True, "tested": 0, "low": current["low"], "high": current["high"]}
               elif current["close"] < current["open"] and momentum > body_size * 0.3:
                   strength = min((body_size / range_size) * 0.5 + 0.3, 1.0) if range_size > 0 else 0.3
                   ob = {"type": "bearish", "level": current["high"], "strength": strength, "valid": True, "fresh": True, "tested": 0, "low": current["low"], "high": current["high"]}
...
class ScalpingEngine:
   ...
   def analyze(self, data, current_price):
       ...
       market_state = {
           ...
           'trend_strength': max(bullish_count, bearish_count) / 4.0,
   ...
   def format_signal_text(self, signal, is_alarm=False):
       if signal.status != "VALID":
           ...
           text = (
               ...
               f"Confluence: {signal.confluence_count}/2\n"
